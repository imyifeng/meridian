...
func TestMemoCRUD(t *testing.T) {
	...
	resp = env.Call("GET", "/api/v1/memos", admin.Token, nil, &list)
	...
	resp = env.Call("GET", "/api/v1/memos/1", admin.Token, nil, &got)
	...
	if resp := env.Call("DELETE", "/api/v1/memos/1", admin.Token, nil, nil); resp.StatusCode != http.StatusNoContent {
		t.Errorf("delete: status %d, want 204", resp.StatusCode)
	}
	if resp := env.Call("GET", "/api/v1/memos/1", admin.Token, nil, nil); resp.StatusCode != http.StatusNotFound {
...
func TestMemoValidation(t *testing.T) {
	...
	for name, body := range cases {
		if resp := env.Call("POST", "/api/v1/memos", admin.Token, body, nil); resp.StatusCode != http.StatusBadRequest {
			t.Errorf("%s: status %d, want 400", name, resp.StatusCode)
		}
	}

	env.Call("POST", "/api/v1/memos", admin.Token, map[string]string{"title": "只标题"}, nil)
	var list struct {
		Memos []map[string]any `json:"memos"`
	}
	env.Call("GET", "/api/v1/memos", admin.Token, nil, &list)
...
func TestMemosAreScopedToTheirOwner(t *testing.T) {
	env := apitest.NewEnv(t)
	admin := env.Admin()
	env.Call("POST", "/api/v1/memos", admin.Token,
		map[string]string{"title": "管理员的备忘录", "body": "私密"}, nil)

	// A second account: no admin-created-users API yet (that is T3), so the
	...
	if resp := env.Call("GET", "/api/v1/memos/1", admin.Token, nil, nil); resp.StatusCode != http.StatusOK {
		t.Errorf("admin lost access to own memo: status %d", resp.StatusCode)
	}
}
...
func TestMemoNotFoundShapes(t *testing.T) {
	...
	if resp := env.Call("GET", "/api/v1/memos/999", admin.Token, nil, nil); resp.StatusCode != http.StatusNotFound {
		t.Errorf("missing memo: status %d, want 404", resp.StatusCode)
	}
	if resp := env.Call("GET", "/api/v1/memos/notanumber", admin.Token, nil, nil); resp.StatusCode != http.StatusNotFound {
...
func TestDataSurvivesRestart(t *testing.T) {
	...
	resp = env.Call("POST", "/api/v1/auth/login", "",
		map[string]string{"username": admin.Username, "password": admin.Password}, &out)
